...
This code contains is a standalone that creates a 'aggregate' file
...
def fill_aggregate_file(agg_hdf5_file,hdf5files,basedir=''):
    """
    Fill a aggregate file from a list of path to existing song HDF5 files.
    If paths are relative, you must provide base dir so we can find them.
    """
    # open aggregate file, get tables
    agg_h5 = HDF5.open_h5_file_append(agg_hdf5_file)
    metadata = summ_h5.root.metadata.songs
    analysis = summ_h5.root.analysis.songs
    path = agg_h5.root.path.songs
    path_row = path.row
    # iterate on hdf5 files
    print 'filling aggregate file with',len(hdf5files),'existing song HDF5 files'
